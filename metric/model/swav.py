...
class ResNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for end_idx in idx_crops:
            _out = self.forward_backbone(torch.cat(inputs[start_idx: end_idx]).to(inputs[start_idx].device))
            if start_idx == 0:
                output = _out
            else:
...
def resnet50(**kwargs):
    ...
    state_dict = load_state_dict_from_url(SWAV_WEIGHTS_URL, progress=True, map_location='cpu')
